...
public final class Bindable<T> {
	 ...
	 * @return an updated {@link Bindable}
	 */
	public Bindable<T> withExistingValue(T existingValue) { // 如果existingValue有值，则返回一个更新后的Bindable对象
		Assert.isTrue(
	...
	private static ResolvableType box(ResolvableType type) {
		...
		if (resolved != null && resolved.isPrimitive()) { // 针对基本类型进行包装处理
		...
		if (resolved != null && resolved.isArray()) { // 针对数组类型进行包装处理
